...
contract AbortOrder is DeOrderTest {
    ...
    function testMoreConfirmIssuerAppendStageIssuerAbortOrder() public{
        ...
        confirmDelivery(issuer, 1,stageIndexs );
        abortOrder(issuer, 1);
        assertEq(address(issuer).balance, 980 ether); //项目结束甲方余额
        assertEq(address(worker).balance, 19 ether);//项目结束乙方余额
    }
    //1個階段劃分，Confirm甲方增加階段劃分，乙方終止任務
