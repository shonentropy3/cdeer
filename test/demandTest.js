...
2. 乙方设置阶段参数 setStageByB
    2.1 订单地址是乙方本人才能确认
    2.2 该订单乙方确认后不能再次确认
    2.4 订单甲方已经确认或者拒绝之后，乙方不能再次确认
    2.5 乙方确认之时需要设置阶段
        a. 设置阶段数不能超过初始最大阶段数（管理员可以设置最大阶段数）
        b. 设置的阶段中总金额需要等于订单显示金额，等于向合约打入的金额
    2.6 乙方确认，订单初始时间改为乙方确认时间，并且每阶段结束时间要延后
        延后时间 = 乙方确认时间 - 甲方确认时间

3. 甲方确认订单 confirmOrderByA
    3.1 确认订单时打入项目款
        a. token和coin都支持
...
5. 终止阶段 terminateStage
    5.1 终止阶段只允许甲乙两人，需终止的阶段不能被确认
    5.2 终止的阶段须是当下时间阶段
    ...
    5.4 阶段终止后，将乙方该阶段获取的金额，甲方剩余项目款分别打给甲乙
...
7. 作为外部接口，订单是否已经接单 isConfirmOrders
